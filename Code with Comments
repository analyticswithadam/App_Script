function getYouTubeComments() {
  const apiKey = 'XXXXXX'; // Replace with your API key
  ...
  do {
    ...
    result.items.forEach(item => {
      ...
      let likeCount = item.snippet.topLevelComment.snippet.likeCount;
      comments.push([author, comment, publishedAt,likeCount]);
    });
...
function insertCommentsToSheet(comments) {
  ...
  headerRange.setValues([['Author', 'Comment', 'Published At','Like Count']]);
